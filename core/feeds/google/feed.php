...
class PGoogleFeed extends PBasicFeed{
  ...
  function formatProduct($product) {
	...
	$output.= $this->formatLine('g:price', sprintf($this->currency_format, $product->regular_price) . $this->currency);
	if ($product->has_sale_price) {
	  $output.= $this->formatLine('g:sale_price', sprintf($this->currency_format, $product->sale_price) . $this->currency);
	}
	...
	if ($this->system_wide_shipping) {
	  ...
	  if (strpos($this->system_wide_shipping_rate, '%') === false) {
	    ...
	    $product->shipping_amount = sprintf("%1.2f", substr($this->system_wide_shipping_rate, 1) * $product->regular_price);
	  }*/
	  $output.= '
		<g:shipping>' .
