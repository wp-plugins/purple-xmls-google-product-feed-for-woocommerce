<?php
/*
  Plugin Name: Purple XMLs Free - Google XML Product Feed for WooCommerce
  Plugin URI: http://www.purpleturtle.pro/
  Description: The Purple XMLs Google Product Feed for WooCommerce creates an XML output by of any category in the WooCommerce installation. It includes all required feed types for Google Merchant Center for non-variable products.
...
if (is_admin()) {
    ...
    function setup_purple_feeds_admin_menu() {
        add_submenu_page('options-general.php', 'Purple Feeds XMLs', 'Purple Feeds XMLs', 'manage_options', 'purple-feeds-xmls', 'purple_feeds_page_settings');
    }
    /**
     * Function to create xml file accourding to user selection
     */
...
function purple_feeds_page_settings() {
    ...
    echo " <div id='poststuff'><div class='postbox' style='width: 98%;'><form name='categories' action='" . site_url() . "' id='categories-xml-form' method='get' target='_blank'>
		...
		<div class='inside export-target'>
		 <table class='form-table' style='width:95%;'>
		  <tbody><tr>";
    echo "<td colspan='3'><h5 for='startdate'>click following button to get categories xml</h5></td>";
    ...
    echo " <div id='poststuff'><div class='postbox' style='width: 98%;'><form name='products'  action='" . site_url() . "' id='cat-product-feeds-xml-form' method='get' target='_blank'>
		<h3 class='hndle'>Products XML Based Category</h3>
		<div class='inside export-target'>
		 <table class='form-table' style='width:95%;'>
		  <tbody><tr>";
    echo "<th><label >Category : </label></th>";
